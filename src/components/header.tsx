'use client';
import { Box, Grid, List, ListItem, GridItem, Flex } from "@chakra-ui/react";
...
export default function Header (){
    ...
    return(
            <Grid as="nav" templateColumns='repeat(6, 1fr)' className="items-center mb-4">
                    <GridItem colSpan={1}>
                    ...
                    <GridItem ref={menuRef} colStart={{base: '3', md: '4'}} colEnd={7} className={`md:block hidden`}>
                        <List className="fixed md:relative w-[100%] top-0 right-0 z-50 h-[100vh] md:h-full bg-gradient-to-r from-[#000000a0] from-0% via-white via-30% to-white to-100% md:bg-none md:pr-4 md:flex justify-between text-lg md:text-sm">
                            <svg onClick={()=>handleClose()} className={`md:hidden fixed top-[5%] right-[5%] cursor-pointer `} width="32" height="31" xmlns="http://www.w3.org/2000/svg"><g fill="#00001A" fill-rule="evenodd"><path d="m2.919.297 28.284 28.284-2.122 2.122L.797 2.419z"/><path d="M.797 28.581 29.081.297l2.122 2.122L2.919 30.703z"/></g></svg>
                            <ListItem><Link href='#' className="hover:text-red-600 absolute top-[16%] left-[35%] text-left md:top-0 md:right-0 md:relative">Home</Link></ListItem>
                            <ListItem><Link href='#' className="hover:text-red-600 absolute top-[23%] left-[35%] md:top-0 md:right-0 md:relative">New</Link></ListItem>
                            <ListItem><Link href='#' className="hover:text-red-600 absolute top-[30%] left-[35%] md:top-0 md:right-0 md:relative">Popular</Link></ListItem>
                            <ListItem><Link href='#' className="hover:text-red-600 absolute top-[37%] left-[35%] md:top-0 md:right-0 md:relative">Trending</Link></ListItem>
                            <ListItem><Link href='#' className="hover:text-red-600 absolute top-[44%] left-[35%] md:top-0 md:right-0 md:relative">Category</Link></ListItem>
                        </List>
                    </GridItem>
                    <GridItem colStart={6} colEnd={7} className="md:hidden justify-self-end">
                        <svg onClick={()=>handleOpen()} className="cursor-pointer" width="40" height="17" xmlns="http://www.w3.org/2000/svg"><g fill="#00001A" fill-rule="evenodd"><path d="M0 0h40v3H0zM0 7h40v3H0zM0 14h40v3H0z"/><path d="M0 0h40v3H0z"/></g></svg>
                    </GridItem>
            </Grid>
    )
}